...
class pgmmanager(context: Context):SQLiteOpenHelper(context, DATABASE_NAME, null , DATABASE_VER) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        db!!.execSQL(("CREATE TABLE $PROGRAM_TABLE ($PROGRAM_ID INTEGER PRIMARY KEY AUTOINCREMENT , $PROGRAM_COMMAND BYTE, $PROGRAM_PGM BYTE , $PROGRAM_NAME TEXT)"))
    }
         ...
         val cursor: Cursor = db.rawQuery(selectQuery, null)
            if(cursor.moveToFirst()){
                do{
                    ...
                    pgm.pgm = cursor.getInt(cursor.getColumnIndex(PROGRAM_ID.toString())).toByte()
                    pgm.name = cursor.getInt(cursor.getColumnIndex(PROGRAM_NAME.toString())).toByte()
